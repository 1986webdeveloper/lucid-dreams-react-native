...
class AppIntroSlider extends React.Component {
  ...
  componentWillMount() {
    this.listener = EventRegister.addEventListener("slideNext", data => {
      this._onNextPress();
    });
  }
  ...
  _OnSubmitData = () => {
    ...
    const { voice } = self.state;
    const { voice_time } = self.state;

    //alert(tags)

    db.transaction(function(tx) {
      tx.executeSql(
        "INSERT INTO table_user (date, time, title, description, rating, first_answer, second_answer, type, fifth_answer, tags, voice_record,voice_time) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
        [
              ...
              [
                {
                  ...
                  onPress: () => {
                    ...
                    self.props.navigation.push("Dashboard");
                  }
                }
              ],
              { cancelable: false }

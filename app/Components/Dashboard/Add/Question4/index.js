...
export default class Question4 extends Component {
      ...
      AudioRecorder.onFinished = data => {
        ...
        if (Platform.OS === "ios") {
          this._finishRecording(
            data.tags_visible === "OK",
            data.audioFileURL,
            data.audioFileSize
          );
          AsyncStorage.setItem("voice", this.state.audioPath);
          AsyncStorage.setItem("voice_time", JSON.stringify(this.state.timer));
        }
      };
    });
  }
  ...
  async _stop() {
    ...
    try {
      ...
      if (Platform.OS === "android") {
        ...
        AsyncStorage.setItem("voice", this.state.audioPath);
        AsyncStorage.setItem("voice_time", this.state.timer);
      }
      return filePath;
    } catch (error) {
      console.error(error);
  ...
  onButtonStart = () => {
    this.requesstAudioPermission();
    if (this.state.recording === false && this.state.playCount === 0) {
    }
  };
  ...
  onStopPress() {
    try {
      ...
      if (this.state.play_icon === false) {
        this.setState({ text_recording: "Playing" });
        // this.onStartTimer()
        // this._resume()
        this._play();
      }
      ...
      <ScrollView style={{ marginTop: "20%" }}>
        ...
        >
          <View
            style={{
              // marginTop: "10%",
              paddingBottom: Platform.OS === "ios" ? 0 : "45%"
            }}
          >

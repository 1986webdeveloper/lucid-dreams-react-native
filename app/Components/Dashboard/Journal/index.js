...
class Journal extends Component {
  ...
  async _play(fileName) {
    if (this.state.play_icon === true) {
      this.setState({
        play_icon: false
      });

      setTimeout(() => {
        var sound = new Sound(this.state.modal_music + "" + "", "", error => {
          if (error) {
      ...
      >
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <TouchableOpacity activeOpacity={1}>
            ...
            {this.state.modal_music === null ? null : (
              <View
                style={{
                  // marginBottom: "3%",
                  marginTop: -20,
